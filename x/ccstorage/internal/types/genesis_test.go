...
func TestCCS_GenesisParams_Validate(t *testing.T) {
	...
	// ok
	{
		param := CurrencyParams{"xfi", 0, "0102", "AABB"}
		require.NoError(t, param.Validate())
	}

	// fail: invalid denom
	{
		param1 := CurrencyParams{"xfi1", 0, "0102", "AABB"}
		require.Error(t, param1.Validate())
	}

	// fail: empty path
	{
		param1 := CurrencyParams{"xfi", 0, "", "AABB"}
		require.Error(t, param1.Validate())

		param2 := CurrencyParams{"xfi", 0, "0102", ""}
		require.Error(t, param2.Validate())
	}

	// fail: invalid hex path
	{
		param1 := CurrencyParams{"xfi", 0, "z", "AABB"}
		require.Error(t, param1.Validate())

		param2 := CurrencyParams{"xfi", 0, "0102", "z"}
		require.Error(t, param2.Validate())
	}
}
