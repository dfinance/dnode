...
var (
	...
	KeyAssets    = []byte("pricefeedassets")
	KeyNominees  = []byte("pricefeednominees")
	KeyPostPrice = []byte("pricefeedpostprice")
)
...
// Params params for price feed. Can be altered via governance
type Params struct {
	Assets    Assets          `json:"assets" yaml:"assets"` //  Array containing the assets supported by the price feed
	Nominees  []string        `json:"nominees" yaml:"nominees"`
	PostPrice PostPriceParams `json:"post_price" yaml:"post_price"`
}

// Posting rawPrices from price feeds configuration params
type PostPriceParams struct {
	// allowed timestamp difference between current block time and price feed's receivedAt (0 - disabled) [sec]
...
// DefaultParams default params for price feed
