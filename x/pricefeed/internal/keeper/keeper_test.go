...
// TestKeeper_SetGetAsset tests adding assets to the price feed, getting assets from the store
func TestKeeper_SetGetAsset(t *testing.T) {
	...
	ap := types.Params{
		Assets: []types.Asset{
			types.Asset{AssetCode: "tstusd", PriceFeeds: types.PriceFeeds{}, Active: true},
		},
	}
	...
	ap = types.Params{
		Assets: []types.Asset{
			types.Asset{AssetCode: "tstusd", PriceFeeds: types.PriceFeeds{}, Active: true},
			types.Asset{AssetCode: "tst2usd", PriceFeeds: types.PriceFeeds{}, Active: true},
		},
	}
	helper.keeper.SetParams(ctx, ap)
	assets = helper.keeper.GetAssetParams(ctx)
...
func TestKeeper_SetAddAsset(t *testing.T) {
	...
	_, found := helper.keeper.GetAsset(ctx, "tstusd")
	require.Equal(t, found, true)
	err := helper.keeper.AddAsset(ctx, helper.addrs[0].String(), "tst2usd", types.Asset{AssetCode: "tst2usd", PriceFeeds: types.PriceFeeds{}, Active: true})
	require.Nil(t, err)
	assets = helper.keeper.GetAssetParams(ctx)
	require.Equal(t, len(assets), 2)
	require.Equal(t, assets[0].AssetCode, "tstusd")
	...
	helper.keeper.AddAsset(ctx, helper.addrs[1].String(), "tst3usd", types.Asset{AssetCode: "tst3usd", PriceFeeds: types.PriceFeeds{}, Active: true})
	assets = helper.keeper.GetAssetParams(ctx)
	require.Equal(t, len(assets), 2)
	require.Equal(t, assets[0].AssetCode, "tstusd")
	require.Equal(t, assets[1].AssetCode, "tst2usd")

	err = helper.keeper.AddPriceFeed(ctx, helper.addrs[0].String(), "tst2usd", helper.addrs[1].Bytes())
	require.Nil(t, err)
	pricefeeds, err := helper.keeper.GetPriceFeeds(ctx, "tst2usd")
	require.Nil(t, err)
	pricefeed := types.NewOracle(helper.addrs[1].Bytes())
	require.Equal(t, pricefeed, pricefeeds[0])

	_, found = helper.keeper.GetAsset(ctx, "nan")
	require.Equal(t, found, false)
}

// TestKeeper_GetSetPrice Test Posting the price by a price feed
func TestKeeper_GetSetPrice(t *testing.T) {
	...
	ap := types.Params{
		Assets: []types.Asset{
			{AssetCode: "tstusd", PriceFeeds: types.PriceFeeds{}, Active: true},
		},
	}
	helper.keeper.SetParams(ctx, ap)

	// Set price by price feed 1
	{
