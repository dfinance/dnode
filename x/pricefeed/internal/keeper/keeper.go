...
// SetPrice updates the posted price for a specific pricefeed
func (k Keeper) SetPrice(
	...
	// find raw price for specified pricefeed
	store := ctx.KVStore(k.storeKey)
	prices := k.GetRawPrices(ctx, assetCode, ctx.BlockHeight())
	var index int
	found := false
	for i := range prices {
		if prices[i].PriceFeedAddress.Equals(pricefeed) {
			index = i
			found = true
			break
		}
	...
	// set the rawPrice for that particular pricefeed
	if found {
		prices[index] = types.PostedPrice{
			AssetCode: assetCode, PriceFeedAddress: pricefeed,
...
// SetCurrentPrices updates the price of an asset to the median of all valid price feed inputs and cleans up previous inputs
func (k Keeper) SetCurrentPrices(ctx sdk.Context) error {
	...
	for _, v := range assets {
		...
		// TODO make threshold for acceptance (ie. require 51% of price feeds to have posted valid prices
		if l == 0 {
			// Error if there are no valid prices in the raw price feed
...
// GetCurrentPrice fetches the current median price of all price feeds for a specific asset
...
// GetRawPrices fetches the set of all prices posted by price feeds for an asset and specific blockHeight
...
// ValidatePostPrice makes sure the person posting the price is an price feed
func (k Keeper) ValidatePostPrice(ctx sdk.Context, msg types.MsgPostPrice) error {
	...
	_, err := k.GetPriceFeed(ctx, msg.AssetCode, msg.From)
	if err != nil {
