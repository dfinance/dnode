...
func TestApp_PostPrice(t *testing.T) {
	...
	pfParams := pricefeed.DefaultParams()
	pfParams.Assets = pricefeed.Assets{
		pricefeed.Asset{
			AssetCode: "uftm",
	...
	pfParams.Nominees = []string{addrs[0].String()}

	keeper.SetParams(ctx, pfParams)
	_, _ = keeper.SetPrice(
		ctx, addrs[0], "uftm",
...
func setUpMockAppWithoutGenesis() (*mock.App, pricefeed.Keeper) {
	...
	pfKeeper := pricefeed.NewKeeper(keyPriceFeed, mapp.Cdc, mapp.ParamsKeeper.Subspace(pricefeed.DefaultParamspace), NewVMStorage())

	// Register routes
	mapp.Router().AddRoute("pricefeed", pricefeed.NewHandler(pfKeeper))
	// Mount and load the stores
	if err := mapp.CompleteSetup(keyPriceFeed); err != nil {
		panic("mock app setup failed")
	}

	return mapp, pfKeeper
}
