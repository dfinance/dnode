...
func (keeper Keeper) AddProposalToQueue(ctx sdk.Context, p types.ExecutableProposal) {
...
func (keeper Keeper) IterateProposalsQueue(ctx sdk.Context, handler func(id uint64, proposal types.ExecutableProposal) (stop bool)) {
...
func (keeper Keeper) ScheduleProposal(ctx sdk.Context, p types.ExecutableProposal) error {
	if p.Data.GetPlan().Height <= ctx.BlockHeight() {
		return sdkErrors.Wrapf(
			sdkErrors.ErrInvalidRequest,
			"update can't be scheduled, planned blockHeight le than current: %d le %d",
			p.Data.GetPlan().Height, ctx.BlockHeight())
	}
